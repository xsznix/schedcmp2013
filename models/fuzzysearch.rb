module FuzzySearch
  ...
  module ClassMethods
    ...
    def act_as_fuzzy_search(*properties)
      ...
      fuzzy_trigram_type = Object::const_get(fuzzy_trigram_type_symbol)
    ...
    def fuzzy_find(words)
      unless words.instance_of? Array
        ...
        words.each do |w|
          word = ' ' + normalize(w) + ' '
          trigrams << (0..word.length-3).collect {|idx| word[idx,3].gsub('\'', ' ')}
        end
        ...
        query = if true 
                  "SELECT count(*) count, #{fuzzy_ref_id} FROM #{fuzzy_ref}_trigrams, #{fuzzy_ref.to_s}s fuzzy_ref WHERE token in ('#{trigrams.join "','"}') and #{fuzzy_ref_id} = fuzzy_ref.id #{conditions} group by #{fuzzy_ref_id} order by count desc"
                else
        ...
        repository.adapter.select(query).collect do |i|
